...
// Connect to db - create db - close connection
var createDB = function(req, res) {
    console.log('Create database entered');

    handleDisconnect();

    // Create Database
    var dbname = req.params.dbname;
    connection.query('CREATE DATABASE ' + dbname, function(err, rows) {
        if (err) {
            console.error(err);
            return;
        }
...
// Connect to db - delete db - close connection
var deleteDB = function(req, res) {
    console.log('Delete database entered');

    handleDisconnect();

    // Create Database
    var dbname = req.params.dbname;
    connection.query('DROP DATABASE ' + dbname, function(err, rows) {
        if (err) {
            console.error(err);
            return;
        }
        console.log('Database ' + dbname + ' delted');
        res.send('Database ' + dbname + 'deleted');
    });
}

// Connect to Database - SELECT query - close connection
...
var query = function(req, res) {
    ...
    connection.query('SELECT ' + selection + ' FROM ' + table,
        function(err, rows) {
            if (err) {
                console.error(err);
                return;
...
// DataBaseCalls
app.get('/dbcreate/:dbname', createDB);
app.get('/dbdelete/:dbname', deleteDB);
